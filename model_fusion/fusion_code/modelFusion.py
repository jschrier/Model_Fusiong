...
#find true positive for single model
def find_STP(df,rank):
    """
    This function calculate the number of true positive in single model base on rank
    rank = the total positive result in model
    """
    TP = 0
    for i in range(len(df)):
        if df.iloc[i][-1] <= rank and df.iloc[i][2] ==1:
            TP += 1
    return TP
...
def find_TP(df, rank):
    """
    This function calculate the number of true positive in combined model base on rank
    rank = the total positive result in model
    """
    TP = 0
    for i in range(len(df)):
...
def RSC_graph(raw):
    ...
    RSCG.plot(grid = True, color = ['c','orange','green','r'])
...
def find_df(combRes, raw, threshold):
    """
    ...
    for i in range(len(combRes)):
        if combRes['rank'][i] <= threshold and raw.loc[i][2] > threshold and raw.loc[i][3] > threshold and raw.loc[i][4] > threshold and raw.loc[i][5] > threshold and raw.loc[i][1]== 1:
...
def find_dfop(combRes, raw, threshold):
    """
    this fucntion compare the fusion model withe all singe model to find new data points 
    which are the truth and only find by the fusion model"""
    result = pd.DataFrame()
    for i in range(len(combRes)):
        if combRes['rank'][i] > threshold and raw.loc[i][1]== 1 and raw.loc[i][2] <= threshold and raw.loc[i][3] <= threshold and raw.loc[i][4] <= 357 and raw.loc[i][5] <= threshold:
            result = result.append(combRes.loc[i])
    print(result)
